...
export interface UnifiedViewHandle {
}

const UnifiedView = React.forwardRef<
	ViewHandle,
	{
	...
	(({ initialFiles, onResolveAllConflicts }, ref) => {
		...
		const [resolvedConflicts, setResolvedConflicts] = React.useState<ConflictResolution[]>([]);

		React.useImperativeHandle(ref, () => ({
			acceptRemote: () => {
				onConflictResolved(0, files[0].remoteContent);
			},
			overwriteRemote: () => {
				onConflictResolved(0, files[0].localContent);
			},
		}));
